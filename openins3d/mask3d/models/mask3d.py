...
from mask3d.models.modules.common import conv, MLP
...
class Mask3D(nn.Module):
    ...
    def forward(
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                if self.train_on_segments:
                    output_iou, output_class, outputs_mask, attn_mask = self.mask_module(
                        queries,
                        mask_features,
                        mask_segments,
                        len(aux) - hlevel - 1,
        ...
        if self.train_on_segments:
            output_iou, output_class, outputs_mask = self.mask_module(
                queries,
                mask_features,
                mask_segments,
                0,
        ...
        return {
            ...
            'pred_iou': predictions_iou[-1],
            "aux_outputs": self._set_aux_loss(
                predictions_class, predictions_mask, predictions_iou
            ),
    ...
    def mask_module(
        ...
        if ret_attn_mask:
            ...
            if point2segment is not None:
                return outputs_iou, outputs_class, output_segments, attn_mask
            else:
                return (
                    outputs_iou, outputs_class,
                    outputs_mask.decomposed_features,
                    attn_mask,
                )

        if point2segment is not None:
            return outputs_iou, outputs_class, output_segments
        else:
            return outputs_iou, outputs_class, outputs_mask.decomposed_features
    ...
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, outputs_iou):
        ...
        return [
            {"pred_logits": a, "pred_masks": b, "pred_iou": c}
            for a, b, c in zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_iou[:-1])
        ]
