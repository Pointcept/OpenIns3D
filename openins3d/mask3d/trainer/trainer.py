...
class InstanceSegmentation(pl.LightningModule):
    ...
    def eval_instance_step(
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            mask_make_it_sparse = torch.from_numpy(self.preds[file_names[bid]]['pred_masks'].astype(np.float16)).to_sparse()
            torch.save(mask_make_it_sparse, os.path.join(pred_save_folder, file_names[bid]+"_masks.pt"))
