                ...
                for m in matches:
                    ...
                    for (gti, gt) in enumerate(gt_instances):
                        ...
                        for pred in gt["matched_pred"]:
                            ...
                            overlap = float(pred['intersection_area']) / (gt['vert_area']+pred['vert_area']-pred['intersection_area'])
...
def make_pred_info(pred: dict):
    ...
    print(pred["pred_classes"].shape, pred["pred_scores"].shape, pred["pred_masks"].shape)
    pred_info = {}
    assert (
...
def assign_instances_for_scan(pred: dict, gt_file: str, all_points):
    ...
    for label in gt2pred:
        for gt in gt2pred[label]:
            gt["matched_pred"] = []
            gt["vert_area"] = instance2boxarea(all_points[gt["mask"]])
    pred2gt = {}
    ...
    for uuid in pred_info:
        ...
        pred_mask_point = all_points[pred_mask, :]
        pred_mask_point_region = instance2boxarea(pred_mask_point)
        ...
        pred_instance['vert_area'] = pred_mask_point_region
        ...
        for (gt_num, gt_inst) in enumerate(gt2pred[label_name]):
            ...
            if intersection > 0:
                ...
                inter_mask = np.logical_and(gt_ids == gt_inst['instance_id'], pred_mask)
                intersection_points = all_points[inter_mask,:]
                # print(intersection_points.shape)
                
                area_intersect = instance2boxarea(intersection_points)
                ...
                gt_copy['intersection_area']   = area_intersect
                pred_copy['intersection_area'] = area_intersect
...
def evaluate_bbox(
    preds: dict, gt_path: str, input_points: list, input_CLASS_LABELS: list, input_VALID_CLASS_IDS: list, output_file: str, dataset: str = "scannet", 
):
    global CLASS_LABELS
    ...
    for i, (k, v) in enumerate(preds.items()):
        ...
        all_points = input_points[i]
        gt2pred, pred2gt = assign_instances_for_scan(v, gt_file, all_points)
        matches[matches_key] = {}
